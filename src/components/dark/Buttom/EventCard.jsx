    ...
    <div className="flex overflow-hidden relative flex-col px-4 pt-4 pb-2.5 aspect-[0.41]">
      ...
      <div className="flex relative gap-1.5 justify-between self-center mt-4 uppercase">
        ...
        <div className="flex gap-1">
          <div>{event.day}</div>
          <div className="shrink-0 w-px bg-white border border-white border-solid h-[13px]" />
          <div>{event.time}</div>
        </div>
      </div>
      <div className="relative mt-5 leading-5 text-neutral-200">
        {event.location}
      </div>
      <div className="relative justify-center px-11 py-3 mt-5 text-sm capitalize bg-black max-md:px-5">
        {event.collection}
      </div>
    </div>
  )
